...
void dcauchy(int n, double *x, double *xl, double *xu, double *A, double *g, double delta, double *alpha, double *s,int trace)
{
/*
c     **********
c
...
*/
	...
	if (interp)
	{
      if(trace)PRINTF("Cauchy Step: Interpolation ...\n");
...
#ifdef EXTRA_LOGIC
                if(nsteps >= MAX_STEPS) search = 0;
#endif
		}	
//        if(trace)PRINTF("Cauchy Step: Interpolation done.\n");        
	}
	else
	{
        if(trace)PRINTF("Cauchy Step: Extrapolation ...\n");
		...
		*alpha = alphas;
		dgpstep(n, x, xl, xu, -(*alpha), g, s);
//        if(trace)PRINTF("Cauchy Step: Extrapolation done\n");
	}
